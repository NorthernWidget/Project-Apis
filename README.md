...
12. Place the box on a measured flat surface and tap the magnet to the marked location by the Hall Effect sensor. This will appropriately zero the offsets for the sensor and increase its near-horizontal accuracy significantly. This must be done when the sensor is powered. From firmware patch 3 the LED lights when the magnet arrives and goes out when the zero is stored: the firmware averages samples at the accelerometer's 10 Hz rate until the mean of every axis has settled (at least 3.2 s, at most 100 s), and it stores nothing if the magnet leaves before then. Hold the magnet still until the LED goes out. When connected to a logger, put the magnet in place, hit the "RESET" button (e.g., on a Margay logger), and hold the magnet in place until the LED goes out. Doing this while connected to a computer is recommended in order to see the first reading on the serial monitor and double check that the zeroing/calibration is appropriate. For a convenient magnet holder, you can use our [3D-printable magnetic wand][3Dprint], which holds a small rare-Earth magnet. This may be a generic part, though this [3/8" x 1/8" Neodymium Disk Magnet](https://www.apexmagnets.com/magnets/3-8-x-1-8-disc-neodymium-magnet) works well in our experience.
...
The quality of any zeroing with the Hall-effect sensor is limited by the accelerometer itself: from firmware patch 3 the zero is an average with about 0.01° of noise, but the LIS3DH's offset drifts by about 0.5 mg per °C, which is about 1.7° across a 60 K season after the zero. The chip's temperature is logged beside pitch and roll (the `AccelT` column) so that a per-unit correction can be fitted once bench data exist; until then, measure and record the orientation of the field-mounted unit by hand as an additional check. The survey behind these numbers is in the private repository NorthernWidget-Skunkworks/Inclinometer-Selection.
